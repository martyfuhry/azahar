...
| **Skip duplicate frames** (`use_skip_duplicate_frames`, under Advanced) | Skips rendering and presenting when the game has not swapped the **top** screen's framebuffer since the last present (`Core::PerfStats::game_frames_updated`, set in `GPU::SetBufferSwap`). Despite the name it never compares frame contents. Saves a render and a present on every second vblank in a 30 fps title. | **No** — and the default changed to **off** on 2026-09-10, in [azahar-emu/azahar#2530](https://github.com/azahar-emu/azahar/pull/2530). A title that updates the screen without a top-screen swap never sets the flag and is never presented at all: [#2495](https://github.com/azahar-emu/azahar/issues/2495) is Inazuma Eleven 3's cutscenes playing their audio over a black screen, on both renderers and on two unrelated GPUs. The fork follows the new default; see `settings-audit.md` §5. If you turn it on for a specific 30 fps title, the repair pass will cede the key to you permanently. |
