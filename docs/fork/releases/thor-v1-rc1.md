...
with the debug key (so it installs alongside nothing else and can be replaced by any later
fork build, but it will not upgrade an APK signed by anyone else).
...
- Every acquire or present result other than success / suboptimal / surface lost / out of
  date hit `UNREACHABLE()` and took the process down as `REASON_CRASH_NATIVE`. The Adreno
  driver answers the first acquire after a wake with `VK_ERROR_DEVICE_LOST` or
  `VK_ERROR_NATIVE_WINDOW_IN_USE_KHR` often enough that this was a routine crash. Results
  are now classified by what could fix them: surface-class results wait for the next
...
- *A re-sent launch intent no longer restarts the game.* `EmulationActivity` is
  ...
  from scratch — and the Thor's launcher re-sends the intent on every lid open. An intent
  targeting the title already running or booting now resumes in place; a genuinely
  different title still goes through the autosave-then-stop-then-reload path.
...
- `-t` is required because the APK is debug-signed; `-d` allows a downgrade if the Thor is
  carrying a build with a higher `versionCode` (which is the build timestamp).
