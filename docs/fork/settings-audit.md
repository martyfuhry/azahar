...
written in the same register as the rest of that directory. Searching upstream while drafting it
turned up better prior art than expected, which raises this from a tidiness complaint to a
probable user-visible bug:

- **#1445 (open)** — "Pokemon X - Certain pokemon have white buggy textures", Adreno, Android,
  no cause identified in the thread. The reporter's own line is *"This color glitch does not
  happen with Citra"*, which is what a silent default divergence looks like from outside.
- **#1292** and **#440** (both closed stale) — the same wrong-colour symptom in Pokémon X/Y.
- **#1136** (closed, `priority - high`) — a different Android bug in the same setting, whose
  reporter noted in passing that *"the config file generated by this build also doesn't seem to
  have values associated with these settings"*, i.e. the blank-key half of this.

Pokémon X/Y are **not** in `hack_list.cpp`'s `ACCURATE_MULTIPLICATION` force list — which covers
Ocarina of Time 3D and three Mario & Luigi titles and no Pokémon game at all — so on Android
they run with it off unless the user finds the switch.

**This is a hypothesis until tested.** Nobody has yet loaded Pokémon X and checked whether the
colours change with the setting. The draft says so plainly and makes it the blocking item on its
verification checklist, because it is the claim that would get the issue dismissed if wrong. The
bare default divergence stands on its own without it.
...
1. **`shaders_accurate_mul`'s Android default contradicts the C++ default** (§3). A plain
   inconsistency in one line that silently costs correctness in titles the per-title hack table
   does not cover, and it is a plausible cause of open **#1445** and of stale **#1292** / **#440**.
   **Drafted: `upstream-issues/12-android-accurate-multiplication-hardcoded-off.md`.** The
   Pokémon connection is a hypothesis and is flagged as the blocking verification item; the
   default divergence is filable on its own regardless.
