...
| **G-2** | Skipped duplicate frames `Flush()` instead of `Finish()` | **SHIPPED**, no measurable effect on a 60 fps title. Would show on a 30 fps title; never tested on one — and as of 2026-09-10 it is **off the default path**: [azahar-emu/azahar#2530](https://github.com/azahar-emu/azahar/pull/2530) disabled `use_skip_duplicate_frames` by default over a correctness bug (#2495) and the fork followed. The change is still correct and still runs for anyone who turns the setting on; it is no longer a candidate for a default-configuration measurement. |
...
| **AS-1** | **Get compression and the file write off the emulation thread.** `SerializeCompressed` (`savestate.cpp`) interleaves walking the object graph, zstd-compressing the result and writing it, all inline in `RunLoop()`, so the emulation thread is stopped for the whole of it and the picture freezes. Only the graph walk genuinely has to be on that thread; a bounded producer/consumer buffer would hand the compress-and-write half to a worker and leave the stall at the serialize alone. **Measured, then built, and it did not work — see `d34c98022` on `fix/autosave-loss`, kept as a do-not-merge commit and reverted.** The split is real: serialize 35-41%, zstd 57-63%, write 1.5-2%, over a uniform ~310 MB of uncompressed input. The bounded-pipe implementation (8 MiB, producer blocks, so peak memory is a constant rather than the 310 MB a buffer would need) is correct and overlaps genuinely — the worker measurably spends its 133-145 ms compressing while the emulation thread serializes. It still buys **nothing**: back to back on one machine, emulation thread blocked 249.8/252.2 ms synchronous against 277.5/244.7 ms with the worker. The reason is copying. The synchronous path copies the 310 MB *once*, because boost serializes straight into the compressor's own input buffer through the streambuf put area; routing it through a pipe adds three more passes over the same 310 MB, and at memory speed that costs about what the compression it was hiding cost. **The general lesson: for one enormous sequential byte stream, moving the bytes between threads costs the same order as the work being moved off, so the split has to be zero-copy to be worth anything.** | Decides whether `autosave_interval` can ever default to on. Also shortens every pause-time save, the lid path included. | M-L | M — a savestate half-written by a worker must still be atomic, and the memory ceiling exists because a New 3DS state is a few hundred MiB uncompressed and must never be held whole | `savestate.cpp` `SerializeCompressed`, `common/zstd_stream.*` | Split the stall into walk vs compress+write on the host first; then H-B `savestate_save_ms`, then `Save completed in N ms` on device |
| **AS-1b** | **Zero-copy off-thread savestate write.** What AS-1 has to become if it is attempted again. The producer's streambuf put area must point directly into the ring, so boost serializes into the shared buffer with no staging copy, and the worker must drive `ZSTD_compressStream2` from the ring's contiguous runs rather than through `ZSTDOutputStreamBuf`, which owns an input buffer of its own. That needs a reserve/commit and peek/consume API on the ring instead of the byte-copy `Write`/`Read` in `d34c98022`, and duplicates a little of `zstd_stream.cpp` in the worker. Expected stall then ~145 ms against ~250, i.e. max(serialize, compress) instead of their sum. | Only route below the ~115-145 ms floor short of AS-3. Decides whether `autosave_interval` can default to on. | L | H — wrap handling in a put area, and a compressor fed from a ring is easy to get subtly wrong in a way that corrupts states rather than crashing | `common/bounded_byte_pipe.*` (in `d34c98022`), `savestate.cpp`, `common/zstd_stream.*` | The A/B in `d34c98022`'s message, repeated: it has to beat 250 ms, and the existing savestate tests plus a save/load round trip have to stay green |
| **AS-3** | **Do not serialize ~310 MB of mostly zeros.** The cost is driven by emulated memory volume, not state size (see settings.h `AutoSaveInterval`), and the volume is ~310 MB whatever the title because FCRAM and VRAM are serialized whole. Skipping pages that are untouched or all-zero would attack the serialize floor *and* the compress time at once, which nothing else on this list does. Feasibility and compatibility cost are written up in the report on `fix/autosave-loss`; it is a format change and therefore gated on **S-1**. | The only thing that makes a savestate cheap rather than cheaper. | L | H — a page wrongly judged untouched is silent save corruption | `memory.cpp`, `savestate.cpp`, `CSTHeader` | Sized, not started |
| **AS-2** | **A cheaper zstd level for autosaves specifically.** Autosaves are transient and rewritten every few minutes; a deliberate savestate is kept. Trading ratio for speed is a much better deal for the former than the latter, and the two currently share one compression level. Measured, on ACNL: level 3 (current) 345.8 ms total / 221.7 zstd / 13.6 MB out; **level 1 256.1 / 135.6 / 14.5 MB** — 26% off the total for 7% more file; level -1 241.3 / 118.0 / 16.8 MB; level -3 232.0 / 108.8 / 18.3 MB; level -9 222.3 / 95.4 / 21.9 MB. Level 1 is the good part of the curve; the negative levels buy little more and cost real ratio on a handheld where the generation ring keeps up to three states per title. Note the second effect, which is what makes this a prerequisite for AS-1 rather than a competitor to it: at level -3 and below the compressor becomes *faster* than the serializer, which is the condition under which a small bounded ring never fills. Do this before AS-1 — it is a constant, not an architecture. | Same as AS-1 and far less risk; may be enough on its own. | S | L — worst case is a bigger file, and `LoadState` does not care what level wrote it | `savestate.cpp` `SerializeCompressed`, `common/zstd_stream.*` | Level sweep on host: `savestate_save_ms` and `savestate_bytes` per level, then confirm on device |
