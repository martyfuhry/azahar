...
| **AS-1** | **Get compression and the file write off the emulation thread.** `SerializeCompressed` (`savestate.cpp`) interleaves walking the object graph, zstd-compressing the result and writing it, all inline in `RunLoop()`, so the emulation thread is stopped for the whole of it and the picture freezes. Only the graph walk genuinely has to be on that thread; a bounded producer/consumer buffer would hand the compress-and-write half to a worker and leave the stall at the serialize alone. **Now measured.** Instrumenting `SerializeCompressed` to time the compressor and the sink separately, and to count the bytes actually fed in: serialize 35-41%, zstd 57-63%, write 1.5-2%, over a uniform ~310 MB of uncompressed input (ACNL 356.6 ms = 124.8 + 224.7 + 7.1; MM3D 330.7 = 117.7 + 205.3 + 7.7; HWL 284.5 = 117.2 + 162.9 + 4.4). So the ceiling on this is real: the stall drops to the serialize alone, ~117-125 ms on host. **But the naive shape does not work.** The uncompressed buffer would be ~310 MB, which is exactly the memory spike this fork exists to avoid, so it has to be a bounded ring the compressor drains concurrently — and at the current level 3 the producer (2.65 GB/s) is *faster* than the consumer (1.4 GB/s), so a small ring just moves the stall onto the compressor and bottoms out near 222 ms instead of 117. **AS-2 is therefore a prerequisite, not an alternative:** the level has to make the compressor keep up before the ring can deliver. | Decides whether `autosave_interval` can ever default to on. Also shortens every pause-time save, the lid path included. | M-L | M — a savestate half-written by a worker must still be atomic, and the memory ceiling exists because a New 3DS state is a few hundred MiB uncompressed and must never be held whole | `savestate.cpp` `SerializeCompressed`, `common/zstd_stream.*` | Split the stall into walk vs compress+write on the host first; then H-B `savestate_save_ms`, then `Save completed in N ms` on device |
