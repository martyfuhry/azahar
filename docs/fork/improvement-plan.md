...
| **AS-1** | **Get compression and the file write off the emulation thread.** `SerializeCompressed` (`savestate.cpp`) interleaves walking the object graph, zstd-compressing the result and writing it, all inline in `RunLoop()`, so the emulation thread is stopped for the whole of it and the picture freezes. Only the graph walk genuinely has to be on that thread; a bounded producer/consumer buffer would hand the compress-and-write half to a worker and leave the stall at the serialize alone. **Nobody has sized the win, and it must be measured before it is attempted:** the obvious decomposition did not add up — `[bench][zstd]` gives ~88 ms per 16 MiB of its own input, but the compressor is fed hundreds of MB to produce a 10-21 MB state, so that figure cannot be projected onto a real save. The host data hints the split is unfavourable (fitting the two extreme host sizes gives ~234 ms fixed plus only ~6 ms/MB, i.e. most of the cost is size-independent and may well be the walk), which if true means this buys far less than it looks like. Measure the split first. | Decides whether `autosave_interval` can ever default to on. Also shortens every pause-time save, the lid path included. | M-L | M — a savestate half-written by a worker must still be atomic, and the memory ceiling exists because a New 3DS state is a few hundred MiB uncompressed and must never be held whole | `savestate.cpp` `SerializeCompressed`, `common/zstd_stream.*` | Split the stall into walk vs compress+write on the host first; then H-B `savestate_save_ms`, then `Save completed in N ms` on device |
| **AS-2** | **A cheaper zstd level for autosaves specifically.** Autosaves are transient and rewritten every few minutes; a deliberate savestate is kept. Trading ratio for speed is a much better deal for the former than the latter, and the two currently share one compression level. Likely the cheapest lever on the autosave stall by a wide margin, and completely unmeasured: a level sweep on the host with `azahar-bench --save-after` across the three benchmark titles would size it in an afternoon, including the disk cost of the worse ratio (which matters here, because the generation ring keeps up to three states per title). Do this before AS-1 — it is a constant, not an architecture. | Same as AS-1 and far less risk; may be enough on its own. | S | L — worst case is a bigger file, and `LoadState` does not care what level wrote it | `savestate.cpp` `SerializeCompressed`, `common/zstd_stream.*` | Level sweep on host: `savestate_save_ms` and `savestate_bytes` per level, then confirm on device |
