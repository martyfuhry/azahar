...
| 13 | `13-zero-area-renderpass-check-tests-height-twice.md` | **none — fixed upstream** in `4034f6c16` (#2544) | was #2510 (merged 2026-09-10) |

Number 13 is withdrawn: do not file it. It was a typo in #2510 (`GetHeight() * GetHeight()`),
found on the 2026-09-10 upstream merge, and upstream fixed it two days later in `4034f6c16`
("video_core: Skip zero-area renderpasses (#2544)"), which replaces the predicate with a new
`Rectangle::GetArea()` and adds the same cull to OpenGL. The draft stays for the record.
