...
bool Handler::ResyncWithHostClock() {
    ...
    if (missed <= std::chrono::seconds::zero()) {
        // The guest is in step with or ahead of the host (fast-forward, or the host clock was set
        // back). Real hardware never moves its clock backwards across a sleep, so neither do we.
