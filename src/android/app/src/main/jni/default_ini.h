...
# Skips display of duplicated frames in 30 fps games. A frame counts as duplicated when the game
# has not swapped the top screen's buffer, not when its contents match, so a title that draws
# without swapping is never presented at all and shows a black screen.
# 0 (default): Off, 1: On
