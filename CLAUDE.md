...
Commit format (mirrors upstream): `subsystem: Imperative summary`, e.g. `core: Add RequestClockResync to Core::System`, `qt: Resync guest clock when resuming emulation`, `tests: Cover clock anchoring at construction`. Subsystems in use: `core`, `qt`, `android`, `tests`, `common`, `video_core`, `audio_core`. Body explains the hardware behavior being matched and the design constraints, not just what changed. See `git log 68236100c..45c7d2cdd` (the clock-resync branch) for the reference style, including how a core change is split into core / frontend / tests commits.
